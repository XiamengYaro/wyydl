# wyydl · 网易云/QQ/哔哩哔哩 歌单定时同步(fnOS NAS)

> 🤖 **本项目为纯 AI Vibe Coding 生成**——全部代码、文档与打包配置均由 AI(编码代理)编写。

定时把指定的网易云音乐歌单同步到 NAS 音乐目录:以**当前账号可用的最高音质**下载,自动写入完整歌曲信息(标签/封面/歌词/流派/厂牌),生成 Jellyfin/Emby 友好的 NFO 元数据,支持四种下载文件布局,并为每个歌单生成 `.m3u8` 播放列表。除普通歌单外还支持**云盘 / 每日推荐 / 私人 FM** 特殊来源,并支持 **QQ 音乐**(直连腾讯接口,扫码登录)与**哔哩哔哩**(直连 API,扫码登录)的歌单、收藏夹与音频下载。无需额外 API 容器。自带 **Web 控制面板**(扫码登录、歌单管理、本地音乐刮削、实时进度、新版本检测)与**飞书/Webhook 通知**。
...
- **工程健壮性**:依赖锁定、单轮下载上限、失败 24h 退避、磁盘空间预检、trash 自动清理、GitHub 新版本检测、CI 自动发布 fpk + GHCR 多架构镜像。
...
`docker-compose.yml` 的 `wyydl-sync` 支持**二选一**:拉取 GHCR 预构建镜像(`image: ghcr.io/xiamengyaro/wyydl-sync:latest`)或本地构建(`build: ./sync`)。需要的挂载:
...
3. **调设置**:音质链、布局、歌词(NFO/逐字)、单轮上限、通知等在「设置」页修改,保存即生效(含 cron 热更新);「添加特殊源」可一键同步云盘/每日推荐/私人 FM。
...
| 区域 | 功能 |
|------|------|
| 顶栏 | 网易/QQ/B站 登录状态(点击 QQ/B站 弹出扫码或 Cookie 登录)、下次运行时间、**立即同步**、**扫码登录** |
| 同步进行中 | 当前阶段、已完成/总数、进度条、**正在下载列表**(每首:百分比 + 已下载/总大小)、最近完成(✓/✗ 与原因) |
| 歌单 | 曲目/已入库/等待/失败四态统计;单歌单同步、移除;点歌单名查看每首歌的入库状态与音质,**多来源引用标记「重复」,失败行可查看原因** |
...
lyrics:
  ...
  yrc: false                   # 优先逐字歌词(自动转 LRC),失败回退普通歌词
...
limits:
  ...
  max_per_run: 0               # 单轮最多下载/重试曲目数;0=不限(超出下轮继续)
  min_free_space: 2            # 音乐目录所在磁盘剩余低于此 GB 时跳过下载
  proxy: ""                    # 可选:CDN 下载代理,如 http://127.0.0.1:7890

playlists:                     # 同步的目标(面板管理)
  - { id: 24381616 }           # 普通歌单:id 必填,name 可选自定义显示名
  # - { source: cloud }        # 特殊源:cloud=云盘 / daily=每日推荐 / fm=私人FM
```
