...
定时把指定的网易云音乐歌单同步到 NAS 音乐目录:以**当前账号可用的最高音质**下载,自动写入完整歌曲信息(标签/封面/歌词/流派/厂牌),生成 Jellyfin/Emby 友好的 NFO 元数据,支持四种下载文件布局,并为每个歌单生成 `.m3u8` 播放列表。除普通歌单外还支持**云盘 / 每日推荐 / 私人 FM** 特殊来源,并支持 **QQ 音乐**(QQMusicApi)与**哔哩哔哩**(yt-dlp,扫码登录)的歌单、收藏夹与音频下载。自带 **Web 控制面板**(扫码登录、歌单管理、本地音乐刮削、实时进度、新版本检测)与**飞书/Webhook 通知**。
